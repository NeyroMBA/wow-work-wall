...
import {
  ...
  UserCheck, Video, MessageCircle, CheckCircle2, ShieldCheck, Database, Globe, LineChart, Lock,
} from "lucide-react";
import {
...
const faqs = [
  {
    q: "Какого масштаба проекты я смогу делать?",
    a: "Масштаб — средний-малый бизнес или подразделение в крупной компании. Условный ориентир — до 100 пользователей. Но зависит ещё и от объёма данных, технических деталей.",
  },
...
const AcceleratorPage = () => {
  ...
  return (
    <div data-theme="accelerator" className="min-h-screen bg-background">
      ...
      <Section id="audience" title="Для" highlight="кого" intro="Вы работаете с людьми и процессами. Пользуетесь ИТ-системами, но не пишете код. И не должны.">
        <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
          <motion.div
            ...
            className="p-6 rounded-xl border border-primary/30 bg-background"
          >
      ...
      <Section
        id="tasks"
        title="Какие задачи уже реально делать с"
        highlight="ИИ"
        intro="Гибридные задачи, когда процесс живёт в нескольких системах. Когда вообще системы нет — учёт ведётся в табличках."
      >
          ...
          {tasks.map((t, i) => {
            ...
            return (
              <motion.div
                ...
                className="flex items-start gap-3 p-5 rounded-xl border border-border bg-background"
              >
      ...
      <Section
        id="solution"
        title="Какое решение мы"
        highlight="предлагаем"
        intro="Программа акселерации: учебные курсы + практика с ментором"
      >
        <div className="max-w-5xl mx-auto">
          <p className="text-center text-sm font-mono font-medium text-primary uppercase tracking-wider mb-6">
            Доступ к курсам
          </p>
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mt-4">
              15 минут: тест и короткое интервью с куратором. Вы получите карту своих текущих компетенций, персональный маршрут из 3–5 шагов и ответ на вопрос, какие задачи из вашего рабочего дня можно автоматизировать первыми.
            </p>
          </motion.div>
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {skills.map((s) => {
                ...
                return (
                  <span
                    key={s.label}
                    className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border border-primary/20 bg-primary/5 text-primary font-medium text-base"
                  >
      ...
      <Section>
        <div className="max-w-3xl mx-auto">
          <motion.div
            initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            className="p-8 rounded-2xl border border-primary/30 bg-primary/5 mb-8 text-center"
          >
          ...
          {/* Временно скрыто: кейсы без превью-картинок */}
