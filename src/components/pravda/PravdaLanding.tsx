...
import trainerImg from "@/assets/pravda/trainer-photo.png";
  ...
  return (
    <div className="flex h-full w-full flex-col gap-2 rounded-[18px] border border-pravda-line bg-pravda-bg p-3">
      ...
      <div className="mt-1 border-t border-pravda-line pt-2">
        ...
        <div className="mt-2 rounded-[10px] border border-pravda-ink bg-pravda-ink px-3 py-2 text-[12px] leading-[1.45] text-pravda-bg">
          <div className="mb-1 font-mono text-[10px] uppercase tracking-[0.18em] text-pravda-bg/70">Ответ агента</div>
          <div>Маржа упала на 4,1 п.п. Два драйвера:</div>
          ...
          <div className="mt-1 text-[11px] text-pravda-bg/80">Действия: пересмотр прайса опта, тендер по поставщику.</div>
        </div>
      </div>
    </div>
  );
...
function Audience() {
  const items = [
    { t: "Директора и владельцы", s: "Хотите наладить регулярный менеджмент и управлять по KPI" },
    { t: "Руководители", s: "Хотите говорить на одном языке с коллегами и топ-менеджерами." },
    { t: "Аналитики", s: "Видите проблемы в данных, но вас не слышат и не понимают." },
  ];
  return (
    <section className="bg-pravda-soft/50 pt-16 pb-10">
      <Container>
        <SectionHead
          title={<>Для тех, кто хочет изменить <span className="whitespace-nowrap">ситуацию</span></>}
        />
