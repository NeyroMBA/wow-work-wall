...
const V6Gallery = ({ onSelect }: Props) => {
    ...
    <section id="gallery" className="relative px-6 md:px-10 py-32">
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="space-y-20">
          {rows.map((row, rIdx) => (
            <div key={rIdx} className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-10">
              {row.map((d, cIdx) => {
                ...
                return (
                  ...
                  >
                    <article
                      ...
                      onClick={() => onSelect(d)}
                    >
                      <div className="v5-frame relative">
                        <span className="v6-tape" aria-hidden />
                        <img src={d.image} alt={d.title} loading="lazy" />
                        <div className="absolute v5-chip" style={{ zIndex: 4 }}>
                          {CATEGORY_LABEL[d.category]}
                        </div>
                        <div className="v6-polaroid-caption">{d.author}</div>
                      </div>
